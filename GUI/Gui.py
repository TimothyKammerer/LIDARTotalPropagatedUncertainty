...
class Gui:
    ...
    def tpuProcessCallback(self):
        ...
        orig_tiles = [f.split('.')[0] for f in os.listdir(self.lasInput.directoryName)
                      if f.endswith('.las')]
        print(orig_tiles)

        split_las_files = [f for f in os.listdir(self.lasSplitTileInput.directoryName)
                           if f.endswith('.las')]
        print(split_las_files)
